...
int main() {
    ...
    run("DeleteObject", HookId::DeleteObject, [&](Autotest& t) {
        // Own brush must survive DeleteObject; an ordinary brush must be deleted.
        HBRUSH own = GetSysColorBrush(COLOR_WINDOW);
        const BOOL ownRet = DeleteObject(own);
        const bool ownAlive = GetObjectType(own) == OBJ_BRUSH;
        const BOOL tmpRet = DeleteObject(created);
        const bool tmpGone = GetObjectType(created) == 0;
        t.valueOk = ownRet && ownAlive && tmpRet && tmpGone;
    });
    DeleteDC(testDc);
